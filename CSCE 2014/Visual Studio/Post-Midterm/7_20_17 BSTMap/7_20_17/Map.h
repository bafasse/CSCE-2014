...
void Map<Key, Value>::insertHelper(const Key& key, const Value& value, Node<Key, Value>*& root)
{
	...
	// Duplicate keys
	// TODO
	else if (key == root->key) root->value = value;
	else if (root->key > key)
		insertHelper(key, value, root->left);
	else insertHelper(key, value, root->right);
}
// ----------------------------------------------------//
template <class Key, class Value>
...
bool Map<Key, Value>::removeHelper(const Key& key, Value& value, Node<Key, Value>*& root)
{
	...
	{
		// Save the current value into the variable 'value'
		// TODO
		value = root->value;
		deleteNode(root);
		return true;
	...
	else return removeHelper(key, value, root->right);
}
// ----------------------------------------------------//
template <class Key, class Value>
...
template <class Key, class Value>
bool Map<Key, Value>::searchHelper(const Key& key, Value& value, const Node<Key, Value>* root) const
{
	...
	else return searchHelper(key, value, root->right);
}
