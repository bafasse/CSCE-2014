...
int hashFn(const string& value)
{
	int num = 71; // Some arbitrary prime number

	for (int i = 0; i < int(value.length()); ++i)
	{
		num = (num * 31) + (value[i] * 31);
	}
	return num;
}

int hashFn(const double value)
{
	// NOT GOING TO BE ON TEST - the C way
	// HERE BE DRAGONS

	// Reinterpret the bits of 'value' as a long (64-bit integer)
	double orig = value;
	long long valueLong = *(long long*)(&orig);
	
	// XOR the left and right halves
	int result = (int)(valueLong >> 32) ^ (int)valueLong;

	return abs(result);
}

template <class T>
int hashFn(const T& val);
...
int HashTable<T>::indexOf(const T& value) const
{
	...
	int index = hashFn(value) % mCapacity;
	for (int i = 0; i < mCapacity; ++i)
	{
